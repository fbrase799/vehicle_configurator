...
| T4 | All project data (configurations + orders) **persisted in a database**. | PRD | SQLite embedded in backend with SQL init script; JPA from the backend. |
...
| T7 | Hibernate **must not** manage schema (`spring.jpa.hibernate.ddl-auto=none`). | Review decision | Schema lives in `backend/src/main/resources/db/001-init.sql` and is the single source of truth. |
...
| O3 | Prototype scope – no SLAs, demo data only, recruiter-friendly URL. | Task-1 "minimal version first" | Two containers; both apps always warm (`minReplicas=1`) in Azure; SQLite embedded in backend. |
...
- **Monorepo layout** mandated in the PRD:
  `backend/`, `frontend/`, `docker/`, `docs/`, `azure/` at the root.
