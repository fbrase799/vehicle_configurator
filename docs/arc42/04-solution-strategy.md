...
| **Persistence** | JPA/Hibernate on embedded SQLite; schema managed **outside** of Hibernate via `backend/src/main/resources/db/001-init.sql`. | DDL versioned with the code, reviewable as SQL, reproducible on every fresh backend start. Hibernate only maps, never migrates. |
...
| **Cloud target** | Azure Container Apps in West Europe; SQLite embedded in backend; both apps `minReplicas=1`. | Demo URL always instantly reachable for recruiters; no separate database container. |
