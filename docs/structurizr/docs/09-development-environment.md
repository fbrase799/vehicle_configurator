...
- <http://localhost:8080/api/> — Spring Boot REST surface (SQLite embedded)
...
| `backend/src/main/resources/db/*.sql` | Applied on first backend start when catalog is empty. `docker compose down -v` drops the SQLite volume, next `up` re-seeds. |
